...
import { arg, send as send$1, getTransactionStatus, decode, encodeMessageFromSignable, config as config$2, resolve, createSignableVoucher, pipe, interaction, block, getEventsAtBlockHeightRange } from '@onflow/sdk';
...
import { INIT as INIT$1, SUBSCRIBE as SUBSCRIBE$1, UNSUBSCRIBE as UNSUBSCRIBE$1, spawn as spawn$1, send, UPDATED as UPDATED$4, SNAPSHOT as SNAPSHOT$2, subscriber as subscriber$1, snapshoter } from '@onflow/util-actor';
...
var getDiscoveryService = function getDiscoveryService() {
  try {
    return Promise.resolve(config$1.first(["discovery.wallet", "challenge.handshake"])).then(function (discoveryWallet) {
      return Promise.resolve(config$1.get("discovery.authn.include", [])).then(function (discoveryAuthnInclude) {
        return Promise.resolve(config$1.first(["discovery.wallet.method", "discovery.wallet.method.default"])).then(function (discoveryWalletMethod) {
          return {
            type: "authn",
...
var configLens = function configLens(regex) {
  try {
    return Promise.resolve(config$1().where(regex)).then(function (_config$where) {
                  ...
                  return _catch$4(function () {
                    return Promise.resolve(execService({
                      ...
                      return Promise.resolve(buildUser(response)).then(function (_buildUser2) {
                        send(NAME$2, SET_CURRENT_USER, _buildUser2);
                      });
                    });
                  }, function (e) {
                      ...
                      return _catch$4(function () {
                        return Promise.resolve(execService({
                          ...
                          return Promise.resolve(buildUser(response)).then(function (_buildUser) {
                            send(NAME$2, SET_CURRENT_USER, _buildUser);
                          });
                        });
                      }, function (e) {
...
var getAccountProofData = function getAccountProofData() {
  try {
    return Promise.resolve(config$1.get("fcl.accountProof.resolver")).then(function (accountProofDataResolver) {
...
var HANDLERS$4 = (_HANDLERS$4 = {}, _HANDLERS$4[INIT$1] = function (ctx) {
  try {
    ...
    return Promise.resolve(config$1.first(["fcl.storage", "fcl.storage.default"])).then(function (storage) {
      var _temp = function () {
        if (storage.can) {
...
}, _HANDLERS$4[SUBSCRIBE$1] = function (ctx, letter) {
  ctx.subscribe(letter.from);
  ctx.send(letter.from, UPDATED$3, _extends$2({}, ctx.all()));
}, _HANDLERS$4[UNSUBSCRIBE$1] = function (ctx, letter) {
...
var spawnCurrentUser = function spawnCurrentUser() {
  return spawn$1(HANDLERS$4, NAME$2);
};
...
var HANDLERS$3 = (_HANDLERS$3 = {}, _HANDLERS$3[INIT$1] = function (ctx) {
  try {
...
}, _HANDLERS$3[SUBSCRIBE$1] = function (ctx, letter) {
  ctx.subscribe(letter.from);
  ctx.send(letter.from, UPDATED$4, ctx.all());
}, _HANDLERS$3[UNSUBSCRIBE$1] = function (ctx, letter) {
  ctx.unsubscribe(letter.from);
}, _HANDLERS$3[SNAPSHOT$2] = function (ctx, letter) {
  try {
...
var getVerifySignaturesScript = function getVerifySignaturesScript(sig, opts) {
  try {
    ...
    return Promise.resolve(config$1.first(["env", "flow.network"])).then(function (network) {
...
var serialize = function serialize(args, opts) {
  ...
  try {
    return Promise.resolve(config$2.first(["sdk.resolve"], opts.resolve || resolve)).then(function (resolveFunction) {
      function _temp2() {
...
var scheduleTick = function scheduleTick(ctx) {
  try {
    ...
    return Promise.resolve(config$2().get("fcl.eventPollRate", RATE)).then(function (_config$get) {
    ...
    var _temp2 = function () {
      if (hwm == null) {
        ...
        return Promise.resolve(block()).then(function (next) {
          ...
          return Promise.resolve(send$1([getEventsAtBlockHeightRange(ctx.self(), hwm.height, next.height - 1)]).then(decode)).then(function (data) {
...
}, _HANDLERS$1[SUBSCRIBE$1] = function (ctx, letter) {
  try {
...
}, _HANDLERS$1[UNSUBSCRIBE$1] = function (ctx, letter) {
  ctx.unsubscribe(letter.from);
...
var addServices = function addServices(services) {
  ...
  try {
    return Promise.resolve(config$1.get("discovery.authn.endpoint")).then(function (endpoint) {
      invariant(Boolean(endpoint), "\"discovery.authn.endpoint\" in config must be defined.");
      return Promise.resolve(config$1.get("discovery.authn.include", [])).then(function (include) {
...
var HANDLERS = (_HANDLERS = {}, _HANDLERS[INIT$1] = function (ctx) {
  try {
...
}, _HANDLERS[SUBSCRIBE$1] = function (ctx, letter) {
  ctx.subscribe(letter.from);
  ctx.send(letter.from, UPDATED, _extends$2({}, ctx.all()));
}, _HANDLERS[UNSUBSCRIBE$1] = function (ctx, letter) {
...
var spawnProviders = function spawnProviders() {
  return spawn$1(HANDLERS, NAME);
};

var authn = {
  subscribe: function subscribe(cb) {
    return subscriber$1(NAME, spawnProviders, cb);
  },
