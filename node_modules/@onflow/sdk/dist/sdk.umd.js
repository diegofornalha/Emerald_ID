(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@onflow/util-logger'), require('@onflow/util-invariant'), require('@onflow/config'), require('@onflow/rlp'), require('@onflow/transport-http'), require('@onflow/util-address'), require('sha3'), require('@onflow/util-template')) :
  typeof define === 'function' && define.amd ? define(['exports', '@onflow/util-logger', '@onflow/util-invariant', '@onflow/config', '@onflow/rlp', '@onflow/transport-http', '@onflow/util-address', 'sha3', '@onflow/util-template'], factory) :
  (global = global || self, factory(global.sdk = {}, null, global.utilInvariant, global.config, global.rlp, global.transportHttp, global.utilAddress, global.sha3, global.utilTemplate));
})(this, (function (exports, logger, utilInvariant, config, rlp, transportHttp, utilAddress, sha3, utilTemplate) {
  function _extends() {
  ...
  var getRefId = function getRefId(opts) {
    try {
      return Promise.resolve(config.config().get("accessNode.api")).then(function (node) {
        return Promise.resolve(config.config.first(["sdk.transport", "sdk.send"], transportHttp.send)).then(function (sendFn) {
      ...
      var _temp4 = function () {
        if (isTransaction(ix) || isScript(ix)) {
          var _temp5 = function _temp5() {
            ...
            return Promise.resolve(config.config().where(/^0x/).then(function (d) {
  ...
  var debug = function debug(key, fn) {
    ...
    return function (ix) {
      try {
        ...
        return Promise.resolve(config.config.get("debug." + key)).then(function (_config$get) {
          var _temp = function () {
  ...
  var send = function send(args, opts) {
    ...
    try {
      return Promise.resolve(config.config.first(["sdk.transport", "sdk.send"], opts.send || transportHttp.send)).then(function (sendFn) {
        ...
        return Promise.resolve(config.config.first(["sdk.resolve"], opts.resolve || resolve)).then(function (resolveFn) {
          ...
          return _opts$node ? _temp(_opts$node) : Promise.resolve(config.config().get("accessNode.api")).then(_temp);
        });
      });
    } catch (e) {
      return Promise.reject(e);
  ...
  var decode = function decode(response) {
    try {
      return Promise.resolve(config.config().where(/^decoder\./)).then(function (decodersFromConfig) {
